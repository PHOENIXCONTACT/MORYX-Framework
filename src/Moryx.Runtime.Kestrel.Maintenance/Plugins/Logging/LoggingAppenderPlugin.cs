...
using Moryx.Logging;
using Moryx.Modules;
using Moryx.Runtime.Kestrel.Maintenance;
using Moryx.Threading;

namespace Moryx.Runtime.Maintenance.Plugins.Logging
{
    [ExpectedConfig(typeof(LoggingMaintenanceConfig))]
    [Plugin(LifeCycle.Singleton, typeof(ILoggingAppender), typeof(ILoggingAppender), Name = PluginName)]
    internal class LoggingAppenderPlugin : ILoggingAppender, ILoggingComponent
    {
        ...
        /// </summary>
        protected LoggingMaintenanceConfig Config { get; set; }

        public void Initialize(LoggingMaintenanceConfig config)
        {
            Config = config;
        }
