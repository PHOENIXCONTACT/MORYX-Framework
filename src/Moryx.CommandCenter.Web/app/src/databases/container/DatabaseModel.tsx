...
class DatabaseModel extends React.Component<DatabaseModelPropsModel & DatabaseModelDispatchPropsModel, DatabaseModelStateModel> {
  ...
  public onConfiguratorTypeChanged(e: React.ChangeEvent<HTMLInputElement>): void {
    ...
    const contextName = contextNameWithoutNamespace(this.state.targetModel);
    config.connectionString = modelConfigurator.connectionStringPrototype.replace(dbNamePlaceholder, contextName);

    const databaseNameEntries = config.properties.subEntries.filter((subEntry: Entry) =>
      subEntry.value.current.includes(dbNamePlaceholder)
    );

    if (databaseNameEntries.length > 0) {
      databaseNameEntries.forEach((entry: Entry) => {
        entry.value.current = entry.value.current.replace(dbNamePlaceholder, contextName);
      });
    }
  ...
  public onConnectionStringChanged(e: React.ChangeEvent<HTMLInputElement>): void {
    ...
    parts.map((part) => {
      ...
      const entry = Object.values(config.properties.subEntries).find((subEntry) => {
        const key = this.state.modelConfigurator.connectionStringKeys[subEntry.identifier];
  ...
  public onInputChanged(e: string, entry: Entry): void {
    ...
    const currentConnectionString = this.state.config.connectionString;
    ...
    // Check if key exists in connection string
    let keyFound = false;
    ...
    if (!keyFound) {
      updatedParts.push(`${key}=${newValue}`);
    }

    // Join back into a single string and ensure it ends with semicolon
    let updatedConnectionString = updatedParts.join(";");
    if (!updatedConnectionString.endsWith(";")) {
      ...
      case TestConnectionResult.Success:
        return (
          <SvgIcon color="success">
            <path d={mdiDatabaseCheckOutline}/>
          </SvgIcon>
        );
              ...
              <GridLegacy item={true} md={12}>
                <Stack spacing={1}>
                  ...
                  <TextField
                    ...
                    placeholder=""
                    value={this.state.config.connectionString}
                    onBlur={() => this.onTestConnection()}
