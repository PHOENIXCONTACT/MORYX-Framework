  ...
  public render(): React.ReactNode {
    return (
      <Collapse in={this.props.IsExpanded}>
        <Grid container={true} size={12} sx={{paddingLeft: 3, paddingRight: 0}}>
          {this.props.IsExpanded &&
            this.props.Entry.subEntries.map((entry, idx) => {
              if (Entry.isClassOrCollection(entry)) {
                return (
                  <Grid container={true} size={12} key={idx}>
                    <Grid size={11} sx={{paddingTop: 0.5, paddingRight: 0}}>
                      <IconButton
                        sx={{padding: 0}}
                        onClick={() => this.toggleCollapsible(entry.uniqueIdentifier ?? entry.identifier)}>
                        <SvgIcon>
                          <path d={this.isExpanded(entry.uniqueIdentifier ?? entry.identifier) ? mdiChevronDown : mdiChevronRight}/>
                        </SvgIcon>
                      </IconButton>
                    ...
                    <Grid size={12}>
                      <Collapse in={this.isExpanded(entry.uniqueIdentifier)}>
                        <Grid sx={{paddingLeft: 3, paddingRight: 0}}>
                ...
                return (
                  <Grid container={true} size={12} key={idx}>
                    <Grid size={11}>{this.preRenderConfigEditor(entry)}</Grid>
          ...
          <Grid container={true} size={12} sx={{paddingTop: 1, paddingBottom: 1}}>
            <Grid size={11}>
              <TextField
                select={true}
                label="Type"
