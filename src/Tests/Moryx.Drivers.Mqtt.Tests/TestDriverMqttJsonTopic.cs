...
namespace Moryx.Drivers.Mqtt.Tests
{
    ...
    public class TestDriverMqttJsonTopic
    {
        ...
        public void Receive_SubscribedTopic_MqttTopicJson_TopicRaisesReceiveEvent()
        {
            ...
            //Act
            _driver.Receive(new MqttApplicationMessage() { Topic = _driver.Identifier + _mqttTopicPascal.Identifier, PayloadSegment = Encoding.ASCII.GetBytes(pascalJson) });
            _driver.Receive(new MqttApplicationMessage() { Topic = _driver.Identifier + _mqttTopicCamel.Identifier, PayloadSegment = Encoding.ASCII.GetBytes(camelJson) });
