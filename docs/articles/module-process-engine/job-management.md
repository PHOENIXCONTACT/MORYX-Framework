...
**Reload after restart:** If the ProcessEngine will be restarted, the JobManager also loads uncompleted jobs from the database. It may happen that a job will be completed after loading it from the database. Therefore loaded jobs will be instantly saved again to filter the completed one's.

**Storage:** The `JobList` publishes an event that for a job is a storage required. This happens whenever a job changed it's state, will be resorted in the list or was completed. The manager is responsible to store these changes.
...
**JobSlots**
The [JobSlots](xref:Moryx.ControlSystem.ProcessEngine.Jobs.JobSlots) class is a helper class to handle the slot size for a machine. Therefore it needs a count for possible parallel runable jobs. It is also possible to define more than one JobSlot object in a Scheduler for example for different production jobs. The helper class provides some helpfull properties like HasSlots or AvailableSlots or methods like `TryResize` or `TryAssign` for the slot handling. So the base slot handling is encapsulated in this class and it is possible to concentrate only on implementing a scheduler behavior which you need for an application.
...
The `IJobDispatcher` helps jobs to handle actions after the start by the JobScheduler. The dispatcher will be used by the job to interrupt, resume, complete, cleanup and abort the production. It is the chain to the process controller. The job dispatcher has also to listen on the process state changes from the activity pool and to forward the changes to the referenced job. The referenced job decides to dispatch a new process or not or handle the information for example to switch to a different state.
...
**Classification**
The classification helps to understand the whole state machine. It will also be used by depending modules to verify a job by its classification instead of each state.
...
**Display Name**
There exist merged synonyms which represent better understandable names for the internal states which will be used for example in the UI.
...
**Endless**
A job can be endless or can be defined with a specific amount. An endless job has usually an amount of 0.
When a job has no amount then it will dispatch processes endlessly until the job is completed manually.
...
The reworked flag is independent from the state but will only be counted if the state switches to success or failure.

### ProductionJob States

![ProductionJob StateMachine](images/production-job-statemachine.png)

**_Important facts_**

1. There are four states which are classified with _Initial_. These are the only ones which should be present after a restart of the ControlSystem.
...
If the process changed to running (activity was dispatched) the amount of the job will be checked. If the amount is reached, the job will be switched to the "Completing" state to finish the last process.
If the amount is not reached, the state will be switched to "Running" and a new process will be dispatched (a process for the queue to have a better performance).

**Running**
In most cases the job will stay in this state until the amount was reached and then will switch to "Completing" to finish the last process.
...
If the job should be restarted and an _Interrupt_ call occurs then the job switches to the state "CompletingInterrupting** to interrupt the current process to have a resumable state after the restart.
...
After the last process is finished the state of the job switches to "Completing" to complete the suspended job.
...
After all processes were interrupted the job switches to the "Interrupted" state.
...
**InterruptedWaiting**
After a restart the successfully interrupted job will be loaded in the "Interrupted" state and waiting for the _Ready_ call.
The interrupted job will switch to the "InterruptedWaiting" state after the _Ready_ call and wait for a start to switch to the "Runnging" state to resume the production.
...
**CleanUp**
After the ControlSystem restart all jobs in the JobList will get a _Load_ call to dispatch new processes.
...
It is also possible that an other restart occurrs so the job can switch back to the "CleanUp" state and wait for a clean up after the restart.

**CleaningUp**
A job switches to the "CleaningUp" state if the clean up must be started.
This means that no new processes will be started and for running processes only unmount activities will be dispatched to remove articles from all depending WPC´s to have a clear production state at the machine.
If everything was cleaned up then the state switches to "Completed".

**Aborting**
This state can be reached from the states which handle processes to abort the current production.
A job in this state cannot be reloaded so a restart will lead to a clean up. An _Abort_ or _Complete_ call will not affect this state. It will abort the process.
...
![SetupJob StateMachine](images/setup-job-statemachine.png)
...
**Running**
With this state, the job has a running process dispatched to the ProcessController. If the process will be finished it will descide how the job will end.

- ProcessState Success
  The process and all setup activities were successful. The setup was completed and then also the job can be completed.
- ProcessState Failue
...
In following there is a description about the sequence of adding a job beginning from the `JobManagementFacade`. The picture shows a whole sequence including the user and the `OrderManagement`.
...
**JobManager**
The JobManager takes the `JobCreationContext` and creates `ProductionJobs` for each recipe and its amount. If the `JobCreationContext` has a defined Position then the `JobManager` tries to get the `JobData` which will be used for the positioning inside of the `JobList`. If there is no Position then the new jobs will be just added at the end of the list.

Before adding each job will be checked with the `JobScheduler` if there is a free slot for producing this job. If there is a free slot then this job is part of the schedulable jobs. Each schedulable job will be sent to every so called `JobHandler` to have the possibility to adjust the jobs which will be added. After handling by the `JobHandler` the schedulable jobs will be added to the `JobList`.
...
For example the `SetupManagement` can add setup jobs if necessary. After every `JobHandler` is done with its work the jobs will be added to the `JobList`. After that each event will be active like `Updated`, `ProgressChanged` or `Removed`.

![Adding a ProductionJob Sequence](images/adding-production-job.png)
