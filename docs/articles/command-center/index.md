...
The Command Center UI is based on the `Moryx.Runtime.Endpoints` project, which provides a REST API for managing the MORYX runtime, modules, and databases. See [Runtime Endpoint](../runtime/endpoint.md) for details on available operations and permissions.
