...
Now we have two plugins which implements the `IConfiguredPlugin` interface with a given config. So we can get a plugin from the `Factory` with a matching configuration.
