...
Please note the [ModelConfiguratorAttribute](../../../src/Moryx.Model/Attributes/ModelConfiguratorAttribute.cs) which is used by MORYX to create a configuration for the context. Here the `NpgsqlModelConfigurator` is using `Npgsql` for PostgreSQL databases.
