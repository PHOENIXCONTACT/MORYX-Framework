# Requirements Engineering with Feature Specifications
...
1. **Define:** Create a new [SPEC-KEY-XXX] entry in the respective `requirements.md` file using the [template](./template.md). Set status to `Accepted`.
...
4. **Reference:** Link the spec key in your subsequent ADRs and Code Commits and switch its status to `Implemented` when you deem it fitting.
5. **Revise:** Alter requirements as the software lifecycle continues and set it's status to `Revised` until implemnetation has happend.

```mermaid
stateDiagram-v2  
   state "PR('File contains Accepted')" as s1  
   state "Accepted" as s2  
   state "Implemented" as s3  
   state "Revised" as s4  

  s1 --> s2: PR Accepted  
  s2 --> s3: PR with implementation merged  
  s3 --> s4: Requirements have been altered  
```
