...
{
    /// <summary>
    /// Database configurator for Products
    /// </summary>
    [ModelConfigurator]
    ...
    {    
        ...
        /// </summary>
        public string TargetModel { get { return "Marvin.Products.Model"; } }
        ...
        public ModelConfigurator() 
        {
            ...
            _container.Register(Classes.FromAssemblyInDirectory(new AssemblyFilter(@".\ModelSetups"))
                               .BasedOn<IModelSetup>()
                               .If(setupType => setupType.GetCustomAttributes<ModelSetupAttribute>(false).Any(att => att.TargetModelNamespace == "Marvin.Products.Model"))
        ...
        public void CreateDatabase(IDatabaseConfig config)
        {
            using (var context = new Entities(BuildConnectionString(config), ContextMode.AllOff))
            {
                ...
                pgConnection = new PgSqlConnection(PgConnectionString(config, true));
                pgConnection.Open();
                pgCommand = new PgSqlCommand(string.Format("INSERT INTO metadata (namespace, version) VALUES ('{0}', {1})", "Marvin.Products.Model", 3), pgConnection); 
                pgCommand.ExecuteNonQuery();
                pgConnection.Close();
        ...
        private class UpdateContext : IUpdateContext
        {
            ...
            {
                var path = "Marvin.Products.Model.Updates." + fileName;
        ...
        private static int GetVersion(IDatabaseConfig config)
        {
            ...
            var command = new PgSqlCommand(string.Format("SELECT version FROM metadata WHERE namespace = '{0}'", "Marvin.Products.Model"), connection);
