...
{
    /// <summary>
    /// Database configurator for UserManagement
    /// </summary>
    [ModelConfigurator]
    ...
    {    
        ...
        /// </summary>
        public string TargetModel { get { return "Marvin.Runtime.UserManagement.Model"; } }
        ...
        public ModelConfigurator() 
        {
            ...
            _container.Register(Classes.FromAssemblyInDirectory(new AssemblyFilter(@".\ModelSetups"))
                               .BasedOn<IModelSetup>()
                               .If(setupType => setupType.GetCustomAttributes<ModelSetupAttribute>(false).Any(att => att.TargetModelNamespace == "Marvin.Runtime.UserManagement.Model"))
        ...
        {
            var context = new UserManagementContainer(BuildConnectionString(config), ContextMode.AllOff);
            try
            {
        ...
        {
            using (var context = new UserManagementContainer(BuildConnectionString(config), ContextMode.AllOff))
            {
                ...
                pgConnection = new PgSqlConnection(PgConnectionString(config, true));
                pgConnection.Open();
                pgCommand = new PgSqlCommand(string.Format("INSERT INTO metadata (namespace, version) VALUES ('{0}', {1})", "Marvin.Runtime.UserManagement.Model", 2), pgConnection); 
                pgCommand.ExecuteNonQuery();
                pgConnection.Close();
        ...
        private class UpdateContext : IUpdateContext
        {
            ...
            {
                var path = "Marvin.Runtime.UserManagement.Model.Updates." + fileName;
        ...
        private static int GetVersion(IDatabaseConfig config)
        {
            ...
            var command = new PgSqlCommand(string.Format("SELECT version FROM metadata WHERE namespace = '{0}'", "Marvin.Runtime.UserManagement.Model"), connection);
        ...
        private static void SetVersion(IDatabaseConfig config)
        {
            ExecuteCommand(config, string.Format("UPDATE metadata SET version = {0} WHERE namespace = '{1}'", 
                                                 2, "Marvin.Runtime.UserManagement.Model"));
        }
        ...
        {
            var context = new UserManagementContainer(BuildConnectionString(config), ContextMode.AllOn);
