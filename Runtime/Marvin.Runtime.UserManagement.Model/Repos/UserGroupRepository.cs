...
{
    internal partial class UserGroupRepository : EntityFrameworkRepository<UserGroup>, IUserGroupRepository
    {
        public static IRepository Create(IUnitOfWork uow, DbContext context)
        {
            var instance = new UserGroupRepository();
            instance.UnitOfWork = uow;
            instance.Context = context;
            instance.DbSet = context.Set<UserGroup>();
            return instance;
        }

        public ICollection<UserGroup> GetAll()
		{
			return DbSet.ToList();
		}
        public virtual UserGroup GetByGroupName(string groupName)
        {
		    return DbSet.FirstOrDefault(e => e.GroupName == groupName);
        }

        public UserGroup Create(string groupName) 
        {
            var entity = Create();
