...
{
    /// <summary>
    /// Database configurator for TestModel
    /// </summary>
    [ModelConfigurator]
    ...
    {    
        ...
        /// </summary>
        public string TargetModel { get { return "Marvin.TestTools.Test.Model"; } }
        ...
        public ModelConfigurator() 
        {
            ...
            _container.Register(Classes.FromAssemblyInDirectory(new AssemblyFilter(@".\ModelSetups"))
                               .BasedOn<IModelSetup>()
                               .If(setupType => setupType.GetCustomAttributes<ModelSetupAttribute>(false).Any(att => att.TargetModelNamespace == "Marvin.TestTools.Test.Model"))
        ...
        public void CreateDatabase(IDatabaseConfig config)
        {
            using (var context = new Entities(BuildConnectionString(config), ContextMode.AllOff))
            {
                ...
                pgConnection = new PgSqlConnection(PgConnectionString(config, true));
                pgConnection.Open();
                pgCommand = new PgSqlCommand(string.Format("INSERT INTO metadata (namespace, version) VALUES ('{0}', {1})", "Marvin.TestTools.Test.Model", 3), pgConnection); 
                pgCommand.ExecuteNonQuery();
                pgConnection.Close();
        ...
        public UpdateSummary UpdateDatabase()
        {
            ...
            if(version < 3)
                throw new Exception(string.Format("Missing update! Last found update: {0} - Model version: {1}", version, 3)); 
        ...
        private class UpdateContext : IUpdateContext
        {
            ...
            {
                var path = "Marvin.TestTools.Test.Model.Updates." + fileName;
        ...
        private static int GetVersion(IDatabaseConfig config)
        {
            ...
            var command = new PgSqlCommand(string.Format("SELECT version FROM metadata WHERE namespace = '{0}'", "Marvin.TestTools.Test.Model"), connection);
